...
import { History, Search, Trash2, Edit2, Download, X, Check } from 'lucide-react';
...
export function HistoryPanel({ onSelectItem }: HistoryPanelProps) {
  const { history, removeFromHistory, clearHistory, renameItem, searchHistory } = useHistory();
  ...
  const downloadItem = (item: QRHistoryItem) => {
    const canvas = document.getElementById(`history-qr-${item.id}`) as HTMLCanvasElement;
    if (!canvas) return;
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete all {history.length} saved QR codes. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast({ title: 'Deleted', description: 'QR code removed from history' });
                    }}
                  >
